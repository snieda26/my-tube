...
// Mock subscribed channels data
const MOCK_SUBSCRIBED_CHANNELS: Channel[] = [
	{
		id: 'ch-1',
		handle: 'tech-insider',
...
export function useMyChannel() {
	return {
		data: MOCK_SUBSCRIBED_CHANNELS[0],
		isLoading: false,
		error: null,
	}
}

export function useChannelVideos(handle: string, params: { page?: number; limit?: number } = {}) {
	return {
		data: { videos: [], total: 0 },
		isLoading: false,
		error: null,
...
export function useToggleSubscription(channelId: string) {
	return {
		mutate: () => {},
		isPending: false,
	}
